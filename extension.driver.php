<?php
	...
	define_safe(MULTILINGUAL_FIELD_NAME, 'Field: Multilingual Tag List');
	define_safe(MULTILINGUAL_FIELD_GROUP, 'multilingual_tag_field');
	...
	class extension_multilingual_tag_field extends Extension {
		...
		public function about() {
			return array(
				'name' => MULTILINGUAL_FIELD_NAME,
				'version' => '1.0',
				'release-date' => '2012-03-28',
				'author' => array(
					array(
						'name' => 'Bajoelcocotero',
						'email' => 'info@bajoelcocotero.com',
				...
				'description'	=> 'Tag field on multilingual basis.'
			);
		}
		...
		public function dAddCustomPreferenceFieldsets($context){
			...
			$label->appendChild(Widget::Input('settings['.MULTILINGUAL_FIELD_GROUP.'][consolidate]', 'yes', 'checkbox', array('checked' => 'checked')));
				...
				foreach ($fields as $field) {
					...
					if ($show_columns) {
						foreach ($show_columns as $column) {
							...
							if ( ($_POST['settings'][MULTILINGUAL_FIELD_GROUP]['consolidate'] !== 'yes') && !in_array($language_code, $new_language_codes)) {
